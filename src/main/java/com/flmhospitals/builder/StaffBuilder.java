...
public class StaffBuilder {
	...
	public static Staff buildStaffFromRegisterStaffDto(RegisterStaffDto registerStaffDto) {
		...
		return Staff.builder()
				...
				.firstName(registerStaffDto.getFirstName()).lastName(registerStaffDto.getLastName())
				.phoneNumber(String.valueOf(registerStaffDto.getPhoneNumber())).gender(registerStaffDto.getGender())
				.dateOfJoining(registerStaffDto.getDateOfJoining()).staffType(registerStaffDto.getStaffType())
				.specialization(registerStaffDto.getSpecialization())
				.experienceInYears(registerStaffDto.getExperienceInYears()).email(registerStaffDto.getEmail())
				.role(registerStaffDto.getRole() != null ? registerStaffDto.getRole().toUpperCase() : null)
				.canLogin(true).isEmployeeActive(true)
				.staffDetails(buildStaffDetailsFromStaffDetailsDto(registerStaffDto.getEmail()))
				.staffAddress(buildStaffAdddressFromStaffAddressDto(registerStaffDto.getStaffAddressDto())).build();

	}

	public static StaffAddress buildStaffAdddressFromStaffAddressDto(StaffAddressDto staffAddressDto) {

		return StaffAddress.builder().landMark(staffAddressDto.getLandmark()).city(staffAddressDto.getCity())
				.state(staffAddressDto.getState()).country(staffAddressDto.getCountry())
				.pinCode(staffAddressDto.getPinCode()).build();
	...
	public static StaffDetails buildStaffDetailsFromStaffDetailsDto(String email) {
		...
		return StaffDetails.builder().email(email).password(hashedPassword).requirePasswordReset(true) // Force password
																										// reset for new
																										// accounts
				.build();
	}
